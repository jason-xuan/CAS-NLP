...
class SinaNewsSpider(CrawlSpider):
    ...
    def parse_news(self, response):
        ...
        title_xpaths = [
            ...
            '//th[@class="f24"]/font/h1/text()',
            '//h1[@id="artibodyTitle"]/text()',
        ]

        item['title'] = ''.join(list(reduce(
                lambda x, y: x + y,
                [response.xpath(xpath).extract() for xpath in title_xpaths]
            )))

        text_xpaths = [
            '//div[@class="article article_16"]/p/text()',
            '//td[@class="l17"]/font/p/text()',
            '//div[@id="artibody"]/p/text()',
        ]

        item['text'] = ' '.join(list(reduce(
                lambda x, y: x + y,
                [response.xpath(xpath).extract() for xpath in text_xpaths]
            ))).replace('\u3000', '')
